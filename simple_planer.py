...
data = Data(directory="data_simple")

dest = {"CDG": 1, "NCE": 2, "ORY": 3, "TLS": 4}

n=len(data.airports['code'])
plane_range = list(range(len(data.aircraft['registration'])+1))
airport_range = list(range(1,n+1))
passenger_range = list(range(len(data.flights['leg_id'])))
time_range = list(range(90))

distances = np.zeros((4,4))
for xkey in dest.keys():
    for ykey in dest.keys():
        if xkey == ykey:
            continue
        else:
            distances[dest[xkey]-1, dest[ykey]-1] = data.routes['distance'][(data.routes['from'] == xkey) & (data.routes['to'] == ykey)]
print(distances)

#distances = np.array([[0,4,4,4],[4,0,4,4],[4,4,4,0]])
#distances

#passenger_destinations = np.array([1,2])
passenger_destinations = np.array([dest[i] for i in data.flights['to']])
print(passenger_destinations)
print(dest)
#print(data.flights['to'])


passenger_start = np.array([dest[i] for i in data.flights['from']])
print(passenger_start)
airplane_start = np.array([dest[i] for i in data.aircraft['position']])
print(airplane_start)
