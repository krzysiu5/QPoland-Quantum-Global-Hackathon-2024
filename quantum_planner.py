...
class QuantumPlanner:
    ...
    def constrain_function(self, P, L, dummy_vars, verbose=False):
        ...
        #    _to_sum.append(np.array(P[t,1:,:]*P[t+1,0:1,:]))
        ...
        _to_sum = []
        for r in self.passenger_range:
            for i in set([0]) | set(self.airport_range):
        ...
        for r in self.passenger_range:
            for t in self.time_range[:-1]:
                for i in set(self.airport_range) - set([self.passenger_destinations[r]]):
                    _to_sum.append(np.sum(-L[r, t, i, :] * L[r, t+1, 0, :]))
        cond4 = np.sum(_to_sum)
        if verbose:
            print("port_to_air", cond4, end=" ")
    ...
    def show_result(self, P_res, L_res, figsize):
        ...
        for plane in self.plane_range[:-1]:
            for t in self.time_range:
                for state in [0] + self.airport_range:
        ...
        ax[0].scatter(positions[:,0], positions[:,1]+positions[:,2]/10, c=positions[:,2],
                    vmin=0, vmax=len(self.plane_range)-1, alpha=0.5)
        ax[0].set_title("plane positions")
        for r in self.passenger_range:
            positions1 = np.array(positions_pass[r])
            ax[r+1].scatter(positions1[:,0], positions1[:,1]+positions1[:,2]/10, c=positions1[:,2],
                            vmin=0, vmax=len(self.plane_range)-1, alpha=0.5)
